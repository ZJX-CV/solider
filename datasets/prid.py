...
class PRID(BaseImageDataset):
    ...
    def __init__(self, root='', verbose=True, split_id=0, **kwargs):
        ...
        self.train = [(img_path, pid, camid,1) for img_path, pid, camid in self.train]
        self.query = [(img_path, pid, camid,1) for img_path, pid, camid in self.query]
        self.gallery = [(img_path, pid, camid,1) for img_path, pid, camid in self.gallery]
        if verbose:
            print("=> PRID loaded")
